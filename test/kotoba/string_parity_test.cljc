(ns kotoba.string-parity-test
  ...
  Since kotoba.string stopped delegating, `clojure.string` is no longer a
...
;; The namespace-has-no-clojure.string-require assertion lived here. It read
;; src/kotoba/lang/text.cljc by path; this repo has no such file, and the
;; property it guarded now belongs to each definition repo rather than to one
;; file. It asserted a layout, never an answer.
